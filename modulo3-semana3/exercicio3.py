def ler_dimensoes_objeto(comandos):
    volume = 0
    lido_sucesso = False

    while not lido_sucesso:
        i = 0
        altura_lida = comandos[i] #input('Altura do objeto (em cm): ')
        i +=1
        altura = validar_medida(altura_lida)
        if altura == -1:
            continue

        comprimento_lido = comandos[i] #input('Comprimento do objeto (em cm): ')
        i +=1
        comprimento = validar_medida(comprimento_lido)
        if comprimento == -1:
            continue

        largura_lida = comandos[i] #input('Largura do objeto (em cm): ')
        i +=1
...
def ler_peso_objeto(comandos):
    lido_sucesso = False
    peso = 0

    while not lido_sucesso:
        peso_lido = comandos #input('Digite o peso do objeto (em Kg): ')
        try:
